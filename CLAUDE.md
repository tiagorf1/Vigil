# Scanner — Claude Code context

This file orients future Claude Code sessions. The full original specification
...
1. **LLM provider is pluggable** (`LLM_PROVIDER` = gemini | anthropic | none),
   not hardcoded to Claude. Default is **Gemini free tier**; `none` is a
...
**Purpose of this document:** Complete specification for Claude Code to build the scanner
...
5. Uses Claude to synthesise each surviving name into a structured report with a suggested
   strategy
...
```
scanner/
├── CLAUDE.md                  ← this file (copy here so Claude Code finds it)
├── README.md
├── .env.example
├── .gitignore
│
...
│   ├── report_generator.py    ← Claude synthesis layer
...
- Model: `claude-opus-4-6` (best reasoning for synthesis). Falls back to
  `claude-sonnet-4-6` if `ANTHROPIC_USE_SONNET=true` in env (cheaper for testing)
...
class ReportGenerator:
    def generate(self, symbol: str, profile: dict, financials: dict, ratios: dict,
        ...
        """
        Calls Claude with a structured prompt and parses the response into:
        {
            "symbol": str,
            "name": str,
            "conviction": int,          # 1-5, Claude's own assessment
...
**System prompt for Claude (use this exactly):**
...
OpenAlice workspaces are Claude Code sessions attached to the OpenAlice runtime.
...
**Why Claude for synthesis rather than a rules engine?**
...
Claude produces the "why this name, why now" narrative that makes the watchlist
actually useful.
